...
TEST(neuron_array_init) {
  ...
  CHECK(arr.v[0] == -65.0f);
  CHECK(arr.u[0] == -13.0f);
  CHECK(arr.spiked[0] == 0);
  CHECK(arr.CountSpikes() == 0);
}
...
TEST(izhikevich_spike) {
  ...
  CHECK(spiked && "Neuron should spike with strong input");
}
...
TEST(izhikevich_updates_last_spike_time) {
  ...
  for (int i = 0; i < 1000; ++i) {
    ...
    if (arr.spiked[0]) {
      CHECK(arr.last_spike_time[0] == t);
      CHECK(arr.last_spike_time[0] != initial_spike_time);
      return;
    }
    arr.i_syn[0] = 0.0f;
  }
  throw std::runtime_error("Should have spiked");
}
...
TEST(izhikevich_nan_recovery) {
  ...
  CHECK(std::isfinite(arr.v[0]));
  CHECK(std::isfinite(arr.u[0]));
}
...
TEST(lif_spike) {
  ...
  CHECK(spiked && "LIF neuron should spike with strong input");
}
...
TEST(synapse_table_csr) {
  ...
  CHECK(table.n_neurons == 3);
  CHECK(table.Size() == 4);
  CHECK(table.row_ptr[1] - table.row_ptr[0] == 2);
  CHECK(table.row_ptr[2] - table.row_ptr[1] == 1);
  CHECK(table.row_ptr[3] - table.row_ptr[2] == 1);
}
...
TEST(spike_propagation) {
  ...
  CHECK(std::abs(i_syn[1] - 2.0f) < 1e-6f);
  CHECK(i_syn[0] == 0.0f);
}
...
TEST(inhibitory_propagation) {
  ...
  table.PropagateSpikes(spiked, i_syn, 1.0f);
  CHECK(std::abs(i_syn[1] - (-3.0f)) < 1e-6f);
}
...
TEST(synapse_no_synapses) {
  ...
  float i_syn[5] = {};
  table.PropagateSpikes(spiked, i_syn, 1.0f);
  for (int i = 0; i < 5; ++i) CHECK(i_syn[i] == 0.0f);
}
...
TEST(synapse_oob_indices) {
  ...
  CHECK(table.Size() == 0 && "OOB index should produce empty table");
...
TEST(stdp_potentiation) {
  ...
  CHECK(syn.weight[0] > original_weight && "Weight should increase (potentiation)");
}
...
TEST(stdp_depression) {
  ...
  CHECK(syn.weight[0] < original_weight && "Weight should decrease (depression)");
}
...
TEST(stdp_weight_bounds) {
  ...
  CHECK(syn.weight[0] >= p.w_min);
  CHECK(syn.weight[0] < w_before_depression && "Depression should reduce weight");
}
...
TEST(stdp_timing_window) {
  ...
  CHECK(syn.weight[0] == 5.0f && "No change outside timing window");
}
...
TEST(loader_neurons_roundtrip) {
  ...
  CHECK(*result == 3);
  CHECK(neurons.n == 3);
  CHECK(neurons.root_id[0] == 100);
  CHECK(neurons.root_id[2] == 300);
  CHECK(neurons.x[1] == 4.0f);
  CHECK(neurons.type[0] == 1);
...
TEST(loader_missing_file) {
  ...
  CHECK(!result.has_value());
  CHECK(result.error().code == ErrorCode::kFileNotFound);
}
...
TEST(loader_truncated_file) {
  ...
  auto result = ConnectomeLoader::LoadNeurons(path, neurons);
  CHECK(!result.has_value());
  CHECK(result.error().code == ErrorCode::kCorruptedData);
...
TEST(config_loader_basic) {
  ...
  auto& cfg = *result;
  CHECK(cfg.name == "test_experiment");
  CHECK(std::abs(cfg.dt_ms - 0.5f) < 0.01f);
  CHECK(std::abs(cfg.duration_ms - 5000.0f) < 0.01f);
  CHECK(cfg.enable_stdp == true);
  CHECK(cfg.bridge_mode == 1);
  CHECK(cfg.connectome_dir == "data/test");
  CHECK(cfg.output_dir == "results/test");
  CHECK(cfg.monitor_neurons.size() == 4);
  CHECK(cfg.monitor_neurons[2] == 2);
  CHECK(cfg.stimulus_protocol.size() == 1);
  CHECK(cfg.stimulus_protocol[0].label == "odor_A");
  CHECK(cfg.stimulus_protocol[0].target_neurons.size() == 3);
...
TEST(config_loader_comments_and_blanks) {
  ...
  auto result = ConfigLoader::Load(path);
  CHECK(result.has_value());
  CHECK(result->name == "after_comments");
...
TEST(config_loader_invalid_dt) {
  ...
  auto result = ConfigLoader::Load(path);
  CHECK(!result.has_value());
  CHECK(result.error().code == ErrorCode::kInvalidParam);
...
TEST(checkpoint_save_load_roundtrip) {
  ...
  CHECK(sim_time2 == 500.0f);
  CHECK(steps2 == 5000);
  CHECK(resyncs2 == 3);
  CHECK(neurons2.v[0] == -50.0f);
  CHECK(neurons2.v[5] == -40.0f);
  CHECK(neurons2.u[3] == -10.0f);
  CHECK(neurons2.dopamine[2] == 0.5f);
  CHECK(neurons2.last_spike_time[7] == 42.0f);
  CHECK(synapses2.weight[1] == 0.99f);
  CHECK(neurons2.type[4] == 3);
  CHECK(neurons2.region[6] == 2);
  CHECK(neurons2.i_ext[1] == 5.0f);
  CHECK(synapses2.HasStochasticRelease());
  CHECK(std::abs(synapses2.p_release[0] - 0.3f) < 1e-6f);
  CHECK(synapses2.HasSTP());
  CHECK(std::abs(synapses2.stp_u[1] - 0.6f) < 1e-6f);
  CHECK(std::abs(synapses2.stp_U_se[0] - 0.4f) < 1e-6f);
...
TEST(connectome_stats_basic) {
  ...
  CHECK(stats.n_neurons == 5);
  CHECK(stats.n_synapses == 5);
  CHECK(stats.n_ach == 2);
  CHECK(stats.n_gaba == 1);
  CHECK(stats.n_glut == 1);
  CHECK(stats.n_da == 1);
  CHECK(stats.max_out_degree == 2);
  CHECK(stats.min_weight == 0.3f);
  CHECK(stats.max_weight == 1.0f);
  CHECK(stats.n_self_loops == 0);
  CHECK(stats.n_out_of_bounds == 0);
  CHECK(stats.n_nan_weights == 0);
}
...
TEST(connectome_stats_isolated_neurons) {
  ...
  ConnectomeStats stats;
  stats.Compute(synapses, neurons);
  CHECK(stats.n_isolated_neurons == 2);
}
...
TEST(stp_recovery_restores_strength) {
  ...
  CHECK(recovered_x > depressed_x + 0.3f);
  CHECK(recovered_x > 0.9f);
}
...
TEST(parametric_gen_stochastic_release) {
  ...
  CHECK(synapses.HasStochasticRelease());
  // All release probabilities should be 0.4
  for (size_t i = 0; i < synapses.p_release.size(); ++i) {
    CHECK(std::abs(synapses.p_release[i] - 0.4f) < 0.001f);
  }
}
...
TEST(synapse_delay_ring_buffer) {
  ...
  CHECK(i_syn[1] == 0.0f);  // not yet delivered
  ...
  std::fill(i_syn, i_syn + 3, 0.0f);
  syn.DeliverDelayed(i_syn);
  CHECK(i_syn[1] > 0.0f);  // delayed current delivered
}
...
TEST(glutamate_sign_inhibitory) {
  ...
  CHECK(SynapseTable::Sign(kGlut) == -1.0f);
  CHECK(SynapseTable::Sign(kGABA) == -1.0f);
  CHECK(SynapseTable::Sign(kACh) == 1.0f);
}
...
TEST(stp_state_clamped) {
  ...
  CHECK(syn.stp_u[0] >= 0.0f && syn.stp_u[0] <= 1.0f);
  CHECK(syn.stp_x[0] >= 0.0f && syn.stp_x[0] <= 1.0f);
}
...
TEST(stp_applied_in_deterministic_propagation) {
  ...
  CHECK(std::abs(i_syn_no_stp[1] - 5.0f) < 1e-6f);
  ...
  CHECK(i_syn_stp[1] > 0.0f);
  CHECK(i_syn_stp[1] < 5.0f);  // modulated below full weight

  // Second spike without recovery: further depression
  float i_syn_stp2[2] = {0.0f, 0.0f};
  syn.PropagateSpikes(spiked, i_syn_stp2, 1.0f);
  CHECK(i_syn_stp2[1] > 0.0f);
  CHECK(i_syn_stp2[1] < i_syn_stp[1]);  // more depressed than first spike
}
...
TEST(stp_deterministic_matches_montecarlo_effective_weight) {
  ...
  CHECK(std::abs(i_det[1] - i_mc[1]) < 1e-6f);
}
...
TEST(synaptic_decay_exponential) {
  ...
  CHECK(std::abs(neurons.i_syn[0] - expected) < 0.01f);
  CHECK(neurons.i_syn[0] > 0.0f);  // not zeroed
  CHECK(neurons.i_syn[1] > 0.0f);

  // Multiple steps should continue decaying
  float before = neurons.i_syn[0];
  neurons.DecaySynapticInput(1.0f, 2.0f);
  CHECK(neurons.i_syn[0] < before);
  CHECK(neurons.i_syn[0] > 0.0f);
}
...
TEST(synaptic_decay_accumulates_with_new_spikes) {
  ...
  syn.PropagateSpikes(neurons.spiked.data(), neurons.i_syn.data(), 1.0f);
  CHECK(neurons.i_syn[1] > 0.0f);
  CHECK(neurons.i_syn[1] < after_first);  // decayed
}
...
TEST(eligibility_trace_accumulates) {
  ...
  CHECK(syn.weight[0] == w_before);
  // But trace should be positive (potentiation)
  CHECK(syn.eligibility_trace[0] > 0.0f);
}
...
TEST(eligibility_trace_converts_with_dopamine) {
  ...
  arr.dopamine[1] = 0.5f;
  EligibilityTraceUpdate(syn, arr, 1.0f, p);
  CHECK(syn.weight[0] > w_before && "Dopamine should convert trace to weight increase");
}
...
TEST(eligibility_trace_decays) {
  ...
  CHECK(std::abs(trace_after - expected) < 0.01f);
}
...
TEST(synaptic_scaling_downscales_overactive_neurons) {
  ...
  CHECK(syn.weight[1] < w_before && "Overactive neuron should have downscaled weights");
}
...
TEST(synaptic_scaling_clamps_range) {
  ...
  CHECK(syn.weight[0] <= w_before * 1.2f + 0.01f);
  CHECK(syn.weight[0] >= w_before * 0.8f - 0.01f);
}
...
TEST(rate_monitor_init) {
  ...
  CHECK(mon.regions.size() == 2);
  CHECK(mon.regions[0].neuron_indices.size() == 50);
  CHECK(mon.regions[1].neuron_indices.size() == 50);
  CHECK(mon.regions[0].name == "KC");
}
...
TEST(rate_monitor_computes_rates) {
  ...
  CHECK(rates[0].rate_hz > 90.0f && rates[0].rate_hz < 110.0f);
}
...
TEST(rate_monitor_literature_lookup) {
  ...
  CHECK(mon.regions[0].ref_min > 0.0f);
  CHECK(mon.regions[0].ref_max <= 10.0f);
}
...
TEST(rate_monitor_in_range_count) {
  ...
  CHECK(RateMonitor::CountInRange(rates) == 1);
}
...
TEST(motor_output_init) {
  ...
  CHECK(motor.HasMotorNeurons());
  CHECK(motor.TotalNeurons() == 10);
  CHECK(motor.descending_left.size() == 3);
  CHECK(motor.avoid_neurons.size() == 2);
}
...
TEST(motor_output_forward_velocity) {
  ...
  CHECK(motor.Command().forward_velocity > 0.0f);
}
...
TEST(motor_output_turning) {
  ...
  CHECK(motor.Command().angular_velocity > 0.0f);
}
...
TEST(motor_output_approach_avoid) {
  ...
  CHECK(motor.Command().approach_drive > 0.0f);
...
TEST(motor_output_freeze_when_silent) {
  ...
  CHECK(motor.Command().freeze > 0.5f);
}
...
TEST(motor_output_from_regions) {
  ...
  CHECK(motor.descending_left.size() == 5);
  CHECK(motor.descending_right.size() == 5);
  CHECK(motor.approach_neurons.size() == 5);
  CHECK(motor.avoid_neurons.size() == 5);
}
...
TEST(homeostasis_init) {
  ...
  CHECK(homeo.bias_current.size() == 100);
  CHECK(homeo.MeanBias() == 0.0f);
}
...
TEST(homeostasis_silent_neurons_get_positive_bias) {
  ...
  CHECK(homeo.MeanBias() > 0.0f);
  CHECK(homeo.FractionExcited() == 1.0f);
}
...
TEST(homeostasis_active_neurons_get_negative_bias) {
  ...
  CHECK(homeo.MeanBias() < 0.0f);
  CHECK(homeo.FractionExcited() == 0.0f);
}
...
TEST(homeostasis_maybe_apply_respects_interval) {
  ...
  for (int i = 0; i < 50; ++i)
    homeo.RecordSpikes(neurons);
  CHECK(!homeo.MaybeApply(neurons));
...
TEST(gap_junction_bidirectional_current) {
  ...
  CHECK(std::abs(arr.i_ext[0] - 10.0f) < 0.01f);   // neuron 0 gains current
  CHECK(std::abs(arr.i_ext[1] - (-10.0f)) < 0.01f); // neuron 1 loses current
}
...
TEST(gap_junction_equal_voltage_no_current) {
  ...
  CHECK(std::abs(arr.i_ext[0]) < 0.001f);
  CHECK(std::abs(arr.i_ext[1]) < 0.001f);
}
...
TEST(stp_init_defaults) {
  ...
  CHECK(syn.HasSTP());
  CHECK(syn.stp_u[0] == 0.5f);
  CHECK(syn.stp_x[0] == 1.0f);
}
...
TEST(stp_recovery_without_spikes) {
  ...
  CHECK(std::abs(syn.stp_x[0] - 1.0f) < 0.01f);
  CHECK(std::abs(syn.stp_u[0] - 0.5f) < 0.01f);
}
...
TEST(stp_reset) {
  ...
  syn.stp_x[5] = 0.01f;
  ResetSTP(syn);
  CHECK(std::abs(syn.stp_u[5] - 0.25f) < 0.001f);
  CHECK(std::abs(syn.stp_x[5] - 1.0f) < 0.001f);
}
...
TEST(stp_presets) {
  ...
  CHECK(fac.U_se < dep.U_se);  // facilitating has lower baseline release
  CHECK(fac.tau_f > dep.tau_f);  // facilitating has slower facilitation decay
  CHECK(com.tau_f > dep.tau_f && com.tau_f < fac.tau_f);
}
...
TEST(cpg_init_splits_groups) {
  ...
  CHECK(cpg.initialized);
  CHECK(!cpg.group_a.empty());
  CHECK(!cpg.group_b.empty());
  // Groups should be non-overlapping
  size_t total = cpg.group_a.size() + cpg.group_b.size();
  CHECK(total == 70);  // 100 - 30% sensory = 70 motor neurons
}
...
TEST(cpg_zero_drive_no_current) {
  ...
  CHECK(std::abs(total_current) < 0.01f);
}
...
TEST(cpg_antiphase_groups) {
  ...
  CHECK(std::abs(sum_a - sum_b) > 0.1f);
}
...
TEST(proprio_haltere_asymmetry) {
  ...
  ProprioMap pm;
  pm.Init(arr, 5, 225.0f);  // midline at 225 so remaining sensory neurons
                             // span both sides after joint/contact/body alloc
  ...
  // With midline=225, some haltere neurons land on each side.
  ...
  CHECK(!pm.haltere_right.empty());
  CHECK(sum_right > sum_left);
}
...
TEST(proprio_zero_state_minimal_current) {
  ...
  CHECK(std::abs(haltere_current) < 0.01f);
}
...
TEST(fingerprint_different_brains_score_low) {
  ...
  CHECK(sim < 0.7f);  // should be noticeably different
}
...
TEST(fingerprint_identity_preserved_threshold) {
  ...
  CHECK(BehavioralFingerprint::IdentityPreserved(ref, candidate, 0.85f));
}
...
TEST(fingerprint_channel_correlation) {
  ...
  CHECK(corr_fwd > 0.99f);   // perfectly correlated
  CHECK(corr_ang < -0.99f);  // perfectly anti-correlated
}
...
TEST(serotonergic_fires_at_low_rate) {
  ...
  // Moderate constant drive (lower drive for slow tonic firing with
  // v_thresh=20 mV serotonergic params)
  for (float t = 0; t < duration_ms; t += 1.0f) {
    neurons.i_ext[0] = 5.0f;
    IzhikevichStep(neurons, 1.0f, sim_time, p);
    spike_count += neurons.spiked[0];
    sim_time += 1.0f;
  }
  float rate_hz = spike_count / (duration_ms / 1000.0f);
  // Should fire tonically; accept a broad range for this cell type
  CHECK(rate_hz > 0.5f && rate_hz < 60.0f);
}
...
TEST(gap_junction_rectification_symmetric) {
  ...
  float tol = 0.01f;
  CHECK(std::abs(neurons.i_ext[0] - 10.0f) < tol);   // current into a
  CHECK(std::abs(neurons.i_ext[1] - (-10.0f)) < tol); // current out of b
}
...
TEST(gap_junction_rectification_attenuates_reverse) {
  ...
  float tol = 0.01f;
  CHECK(std::abs(neurons.i_ext[0] - (-3.0f)) < tol);
  CHECK(std::abs(neurons.i_ext[1] - 3.0f) < tol);
}
...
TEST(gap_junction_rectification_forward_unattenuated) {
  ...
  float tol = 0.01f;
  CHECK(std::abs(neurons.i_ext[0] - 10.0f) < tol);
  CHECK(std::abs(neurons.i_ext[1] - (-10.0f)) < tol);
}
...
TEST(gap_junction_fully_rectifying) {
  ...
  float tol = 0.01f;
  CHECK(std::abs(neurons.i_ext[0]) < tol);
  CHECK(std::abs(neurons.i_ext[1]) < tol);
}
...
TEST(nwb_export_creates_files) {
  ...
  CHECK(nwb.total_spikes == 1);
  CHECK(nwb.n_recorded_steps == 2);
...
TEST(nwb_export_voltage_traces) {
  ...
  CHECK(header.find("neuron_0") != std::string::npos);
  CHECK(header.find("neuron_3") != std::string::npos);
  CHECK(header.find("neuron_7") != std::string::npos);
  fclose(vf);
...
TEST(nwb_export_stimulus_metadata) {
  ...
  CHECK(json.find("odor_A") != std::string::npos);
  CHECK(json.find("electric shock") != std::string::npos);
  CHECK(json.find("NWBFile") != std::string::npos);
...
// CellTypeLabel is private to NWBExporter, tested indirectly via NWB export.

// ---- Per-NT synaptic time constants ----

TEST(per_nt_tau_ach_fast) {
  CHECK(SynapseTable::TauForNT(NTType::kACh) == 2.0f);
}

TEST(per_nt_tau_gaba_slow) {
  CHECK(SynapseTable::TauForNT(NTType::kGABA) == 5.0f);
}

TEST(per_nt_tau_glut_moderate) {
  CHECK(SynapseTable::TauForNT(NTType::kGlut) == 3.0f);
}
...
TEST(assign_per_neuron_tau_from_synapses) {
  ...
  CHECK(neurons.tau_syn.size() == 3);
  CHECK(neurons.tau_syn[0] == 2.0f);  // ACh dominant
  CHECK(neurons.tau_syn[1] == 5.0f);  // GABA dominant
  CHECK(neurons.tau_syn[2] == 3.0f);  // Glut only
}
...
TEST(per_neuron_decay_uses_tau_syn) {
  ...
  float tol = 0.001f;
  CHECK(std::abs(neurons.i_syn[0] - expected_0) < tol);
  CHECK(std::abs(neurons.i_syn[1] - expected_1) < tol);
  // Fast ACh should decay more than slow GABA
  CHECK(neurons.i_syn[0] < neurons.i_syn[1]);
}
...
TEST(sim_features_default) {
  ...
  CHECK(f.izhikevich);
  CHECK(!f.stdp);
  CHECK(!f.structural_plasticity);
  CHECK(f.homeostasis);
  CHECK(f.gap_junctions);
}
...
TEST(sim_features_full_preset) {
  ...
  CHECK(f.stdp);
  CHECK(f.structural_plasticity);
  CHECK(f.short_term_plasticity);
  CHECK(f.stochastic_release);
  CHECK(f.recording);
  CHECK(f.CountEnabled() == 22);
}
...
TEST(sim_features_inference_preset) {
  ...
  CHECK(!f.homeostasis);
  CHECK(!f.gap_junctions);
  CHECK(!f.neuromodulation);
  CHECK(f.izhikevich);
  CHECK(f.spike_propagation);
}
...
TEST(temperature_disabled_returns_one) {
  TemperatureModel tm;
  tm.enabled = false;
  CHECK(tm.ChannelScale() == 1.0f);
  CHECK(tm.SynapseScale() == 1.0f);
  CHECK(tm.MembraneScale() == 1.0f);
}
...
TEST(temperature_at_reference_returns_one) {
  ...
  float tol = 0.001f;
  CHECK(std::abs(tm.ChannelScale() - 1.0f) < tol);
  CHECK(std::abs(tm.SynapseScale() - 1.0f) < tol);
}
...
TEST(temperature_higher_speeds_up) {
  ...
  float tol = 0.01f;
  CHECK(std::abs(tm.ChannelScale() - 2.5f) < tol);
  // Q10=2.0 for synapses: should scale by 2.0x
  CHECK(std::abs(tm.SynapseScale() - 2.0f) < tol);
}
...
TEST(temperature_lower_slows_down) {
  ...
  float tol = 0.01f;
  CHECK(std::abs(tm.ChannelScale() - 0.4f) < tol);
}
...
TEST(sfa_init) {
  ...
  sfa.Init(10);
  CHECK(sfa.initialized);
  CHECK(sfa.calcium.size() == 10);
  CHECK(sfa.MeanCalcium() == 0.0f);
}
...
TEST(sfa_spike_increases_calcium) {
  ...
  CHECK(sfa.calcium[0] > 0.0f);
  CHECK(sfa.calcium[1] == 0.0f);
}
...
TEST(sfa_calcium_decays) {
  ...
  CHECK(sfa.calcium[0] < ca_after_spike * 0.05f);
}
...
TEST(sfa_reduces_firing_rate) {
  ...
  for (int t = 0; t < 2000; ++t) {
    n2.i_ext[0] = drive;
    sfa.Update(n2, 1.0f);           // apply sAHP current before step
  ...
  CHECK(spikes_with_sfa < spikes_no_sfa);
  // But not eliminate them entirely
  CHECK(spikes_with_sfa > 0);
}
...
TEST(distance_delay_basic) {
  ...
  CHECK(syn.HasDelays());
  CHECK(syn.delay_steps[0] == 20);  // 2.0ms / 0.1ms = 20 steps
}
...
TEST(distance_delay_multiple_synapses) {
  ...
  CHECK(syn.delay_steps[0] == 10);  // 0->1: 1ms
  CHECK(syn.delay_steps[1] == 50);  // 0->2: 5ms
  // Ring buffer should be sized for the max delay
  CHECK(syn.ring_size == 51);
}
...
TEST(distance_delay_large_delay_uint16) {
  ...
  CHECK(syn.delay_steps[0] == 1500);  // 150ms / 0.1ms
  CHECK(syn.delay_steps[0] > 255);    // proves uint8_t would overflow
  CHECK(syn.ring_size == 1501);
}
...
TEST(distance_delay_velocity_scaling) {
  ...
  CHECK(syn_slow.delay_steps[0] == 200);  // slow unmyelinated
  CHECK(syn_fast.delay_steps[0] == 10);   // fast myelinated
  CHECK(syn_slow.delay_steps[0] > syn_fast.delay_steps[0]);
}
...
TEST(distance_delay_ring_buffer_delivery) {
  ...
  syn.PropagateSpikes(spiked, i_syn, 1.0f);
  syn.AdvanceDelayRing();
  CHECK(i_syn[1] == 0.0f);  // not delivered yet
  ...
  for (int step = 1; step < 10; ++step) {
    ...
    syn.PropagateSpikes(spiked, i_syn, 1.0f);
    syn.AdvanceDelayRing();
    CHECK(i_syn[1] == 0.0f);  // still waiting
  }
...
TEST(conduction_velocity_constants) {
  ...
  static_assert(SynapseTable::kVelocityDrosophilaUnmyel < SynapseTable::kVelocityMammalUnmyel);
  static_assert(SynapseTable::kVelocityMammalUnmyel < SynapseTable::kVelocityMammalThinMyel);
  static_assert(SynapseTable::kVelocityMammalThinMyel < SynapseTable::kVelocityCorpusCallosum);
  static_assert(SynapseTable::kVelocityCorpusCallosum < SynapseTable::kVelocityMammalThickMyel);

  // Verify magnitudes are in physiological range
  static_assert(SynapseTable::kVelocityDrosophilaUnmyel >= 0.1f);
  static_assert(SynapseTable::kVelocityMammalThickMyel <= 200.0f);
}
...
TEST(compartmental_array_init) {
  ...
  arr.Resize(10);
  CHECK(arr.n == 10);
  CHECK(arr.v_soma[0] == -65.0f);
  CHECK(arr.v_apical[0] == -65.0f);
  CHECK(arr.v_basal[0] == -65.0f);
  CHECK(arr.Ca_i[0] > 0.0f);  // resting calcium
  CHECK(arr.spiked[0] == 0);
  CHECK(arr.CountSpikes() == 0);
}
...
TEST(compartmental_resting_stability) {
  ...
  // Soma should stay within 20mV of initial resting potential.
  // The multi-compartment coupling and active channels cause some
  // settling from the initial conditions toward the true steady state.
  float drift = std::abs(arr.v_soma[0] - v0);
  CHECK(drift < 20.0f);
  CHECK(std::isfinite(arr.v_soma[0]));
  CHECK(std::isfinite(arr.v_apical[0]));
  CHECK(std::isfinite(arr.v_basal[0]));
  CHECK(arr.Ca_i[0] >= 0.0f);
}
...
TEST(compartmental_bap_propagation) {
  ...
  CHECK(arr.bap_apical_remaining[0] > 0.0f);
  CHECK(arr.bap_basal_remaining[0] > 0.0f);
  ...
  CHECK(va_after > va_before_spike - 5.0f || arr.bap_apical_remaining[0] > 0.0f);
  CHECK(vb_after > vb_before_spike - 5.0f || arr.bap_basal_remaining[0] > 0.0f);
}
...
TEST(compartmental_nmda_dendritic_activation) {
  ...
  CHECK(arr.s_NMDA_apical[0] > 0.0f);
  CHECK(arr.s_NMDA_apical[1] > 0.0f);
  ...
  CHECK(B1 > B0);  // confirms depolarized dendrite gets more NMDA current
}
...
TEST(compartmental_ih_depolarizes_at_rest) {
  ...
  CHECK(n0.v_apical[0] > n1.v_apical[0] - 0.5f);
}
...
TEST(compartmental_default_params_valid) {
  ...
  CHECK(p.soma.Cm > 0.0f);
  CHECK(p.apical.Cm > p.soma.Cm);  // spine correction
  CHECK(p.channels.g_Na > p.channels.g_Na_basal);  // soma has higher Na
  CHECK(p.channels.g_CaHVA > 0.0f);
  CHECK(p.v_thresh > p.v_reset);
  CHECK(p.refractory_ms > 0.0f);

  // L2/3 should have less Ca than L5
  CHECK(p23.channels.g_CaHVA < p.channels.g_CaHVA);

  // Area fractions should sum to 1
  float total = p.soma.area_frac + p.apical.area_frac + p.basal.area_frac;
  CHECK(std::abs(total - 1.0f) < 0.01f);
}
...
TEST(compartmental_no_nan_divergence) {
  ...
  for (int step = 0; step < 10000; ++step) {
    ...
    for (size_t i = 0; i < arr.n; ++i) {
      CHECK(std::isfinite(arr.v_soma[i]));
      CHECK(std::isfinite(arr.v_apical[i]));
      CHECK(std::isfinite(arr.v_basal[i]));
      CHECK(std::isfinite(arr.Ca_i[i]));
      CHECK(arr.Ca_i[i] >= 0.0f);
    }
  }
}

TEST(compartmental_multiple_spikes) {
  // With sustained input, the neuron should fire at least once.
  CompartmentalArray arr;
  arr.Resize(1);
  auto p = DefaultPyramidalParams();
  ...
  for (int step = 0; step < 40000; ++step) {
    arr.i_ext_soma[0] = 20.0f;  // strong drive to overcome channel conductances
    CompartmentalStep(arr, 0.025f, step * 0.025f, p);
    if (arr.spiked[0]) spike_count++;
  }

  // Should produce at least one spike over 1000ms with strong drive
  CHECK(spike_count >= 1);
}
...
TEST(compartmental_boltzmann_and_bell) {
  ...
  CHECK(std::abs(t_peak - 10.0f) < 0.01f);
...
TEST(istdp_init) {
  InhibitorySTDP rule;
  CHECK(!rule.IsInitialized());
  rule.Init(100, 10);
  CHECK(rule.IsInitialized());
  CHECK(rule.x_pre.size() == 100);
  CHECK(rule.x_post.size() == 10);
}
...
TEST(istdp_alpha_computation) {
  ...
  CHECK(std::abs(alpha - 0.2f) < 1e-6f);
}
...
TEST(istdp_skips_excitatory_synapses) {
  ...
  InhibitorySTDPUpdate(syn, neurons, 1.0f, rule);
  CHECK(syn.weight[0] == 1.0f);  // unchanged
}
...
TEST(neuromod_effects_oa_synaptic_gain) {
  ...
  CHECK(std::abs(neurons.i_syn[0] - 13.0f) < 0.01f);
}
...
TEST(neuromod_effects_below_threshold_ignored) {
  ...
  fx.Apply(neurons);
  CHECK(neurons.i_ext[0] == 0.0f);
  CHECK(neurons.i_syn[0] == 10.0f);
}

TEST(sim_features_new_flags_counted) {
  SimFeatures f = SimFeatures::Full();
  CHECK(f.inhibitory_plasticity == true);
  CHECK(f.neuromodulator_effects == true);
  CHECK(f.nmda == true);
  // Verify counting includes new flags
  int count = f.CountEnabled();
  CHECK(count >= 19);  // at least 19 features in Full mode (including NMDA)
}
...
TEST(nmda_mg_block_voltage_dependence) {
  ...
  // Always in [0, 1]
  CHECK(B_rest >= 0.0f && B_rest <= 1.0f);
  CHECK(B_pos >= 0.0f && B_pos <= 1.0f);
}
...
TEST(nmda_coincidence_detection) {
  ...
  CHECK(B_dep > 3.0f * B_rest);  // depolarized should get >3x more
}
...
TEST(nmda_calcium_accumulates_and_decays) {
  ...
  CHECK(ca_after < ca_peak * 0.25f);
  CHECK(ca_after > 0.0f);  // not fully gone yet due to accumulated baseline
}
...
TEST(nmda_no_current_at_rest) {
  ...
  CHECK(B < 0.06f);
  CHECK(arr.i_ext[0] < 1.0f);  // minimal current despite large g
}
...
TEST(scoped_timer_basic) {
  ...
  CHECK(t.elapsed_ms() >= 0.0);
  CHECK(t.elapsed_us() >= 0.0);
}
...
TEST(isi_stats_regular) {
  ...
  CHECK(stats.n_intervals == 19);
  CHECK(std::abs(stats.mean_ms - 50.0f) < 0.1f);
  CHECK(std::abs(stats.median_ms - 50.0f) < 0.1f);
  CHECK(stats.cv < 0.01f);  // perfectly regular
  CHECK(stats.is_regular());
}
...
TEST(isi_stats_bursty) {
  ...
  auto stats = ComputeISI(times);
  CHECK(stats.n_intervals == 14);
  CHECK(stats.cv > 1.0f);  // bursty
  CHECK(stats.is_bursty());
}
...
TEST(burst_detection) {
  ...
  CHECK(bursts.size() == 3);
  CHECK(bursts[0].n_spikes == 4);
  CHECK(std::abs(bursts[0].duration_ms() - 9.0f) < 0.1f);

  auto bstats = ComputeBurstStats(bursts, 300.0f);
  CHECK(bstats.n_bursts == 3);
  CHECK(std::abs(bstats.mean_ibi_ms - 100.0f) < 1.0f);
}
...
TEST(spike_collector) {
  ...
  CHECK(collector.TotalSpikes() == 3);
  CHECK(collector.trains[0].size() == 1);
  CHECK(collector.trains[5].size() == 1);
  CHECK(collector.trains[3].size() == 1);

  auto pop = collector.GetPopulationTrain(0, 10);
  CHECK(pop.size() == 3);
  CHECK(pop[0] == 10.0f);  // sorted
}
...
TEST(oscillation_detection) {
  ...
  CHECK(period > 35.0f && period < 45.0f);
}
...
TEST(memory_tracker) {
  ...
  CHECK(tracker.TotalBytes() == 100000 * 4 + 100000 * 4 + 500000 * 4);
  CHECK(tracker.CategoryBytes("neurons") == 100000 * 4 * 2);
  CHECK(tracker.CategoryBytes("synapses") == 500000 * 4);

  tracker.Untrack("synapses");
  CHECK(tracker.CategoryBytes("synapses") == 0);
  CHECK(tracker.PeakBytes() == 100000 * 4 + 100000 * 4 + 500000 * 4);
