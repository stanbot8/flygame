...
TEST(lod_transition_init) {
  ...
  assert(engine.grid.nx > 0);
  assert(engine.grid.ny > 0);
  assert(engine.grid.nz > 0);
  assert(engine.lod.region_lods.size() == 2);
  assert(engine.chunks.empty());
  assert(engine.ch_sdf != SIZE_MAX);
  assert(engine.ch_region != SIZE_MAX);
  assert(engine.field.ch_e != SIZE_MAX);
  assert(engine.field.ch_i != SIZE_MAX);
}
...
TEST(lod_transition_escalate_basic) {
  ...
  assert(ok);
  assert(engine.chunks.size() == 1);
  assert(engine.chunks[0].name == "region_A");
  assert(engine.chunks[0].neurons.n > 0);
  assert(engine.chunks[0].synapses.Size() > 0);
  assert(engine.lod.region_lods[0].current_lod == LODLevel::kNeuron);
}
...
TEST(lod_transition_escalate_neurons_initialized) {
  ...
  for (size_t i = 0; i < chunk.neurons.n; ++i) {
    ...
    assert(chunk.neurons.v[i] >= v_rest);
    assert(chunk.neurons.v[i] <= chunk.params.v_thresh);
  }
  mean_v /= static_cast<float>(chunk.neurons.n);

  // Mean voltage should be above resting (we set E=0.3)
  assert(mean_v > v_rest + 1.0f);
}
...
TEST(lod_transition_deescalate) {
  ...
  assert(ok);
  assert(engine.chunks.empty());
  assert(engine.lod.region_lods[0].current_lod == LODLevel::kContinuum);
...
TEST(lod_transition_escalate_both_regions) {
  ...
  assert(ok1);
  assert(ok2);
  assert(engine.chunks.size() == 2);
  assert(engine.TotalActiveNeurons() > 0);

  // De-escalate one, keep the other
  engine.DeEscalate(0);
  assert(engine.chunks.size() == 1);
  assert(engine.chunks[0].name == "region_B");
}
...
TEST(lod_transition_step_runs) {
  ...
  // Should have accumulated spikes
  auto& chunk = engine.chunks[0];
  assert(chunk.step_count > 0);

  // Neurons should have valid voltages
  for (size_t i = 0; i < chunk.neurons.n; ++i) {
    assert(std::isfinite(chunk.neurons.v[i]));
  }
}
...
TEST(lod_transition_automatic_lod) {
  ...
  assert(engine.lod.region_lods[1].current_lod == LODLevel::kContinuum);
}
...
TEST(lod_transition_total_neurons_and_spikes) {
  ...
  assert(total > engine.chunks[0].neurons.n);  // both chunks contribute
}
...
TEST(lod_transition_escalate_deescalate_cycle) {
  ...
  // Neurons should still have valid state
  for (size_t i = 0; i < engine.chunks[0].neurons.n; ++i) {
    assert(std::isfinite(engine.chunks[0].neurons.v[i]));
  }
}
...
TEST(lod_transition_field_still_evolves) {
  ...
  for (size_t i = 0; i < engine.grid.NumVoxels(); ++i) {
    assert(std::isfinite(engine.grid.channels[engine.field.ch_e].data[i]));
    assert(std::isfinite(engine.grid.channels[engine.field.ch_i].data[i]));
  }
}
...
TEST(multiscale_drosophila_init) {
  ...
  assert(engine.initialized);
  assert(engine.brain_sdf.primitives.size() >= 10);
  assert(engine.lod.region_lods.size() == engine.brain_sdf.primitives.size());
  assert(engine.grid.nx > 0);

  // Verify field has activity inside the brain
  float E_center = engine.ReadActivity(250, 150, 100);
  assert(E_center >= 0.0f);
}
...
TEST(multiscale_drosophila_escalate_antennal_lobe) {
  ...
  assert(engine.chunks[0].neurons.n > 0);
  assert(engine.chunks[0].synapses.Size() > 0);
  assert(engine.lod.region_lods[al_idx].current_lod == LODLevel::kNeuron);
}
...
TEST(multiscale_drosophila_focus_driven_escalation) {
  ...
  if (olr_idx >= 0) {
    assert(engine.lod.region_lods[olr_idx].current_lod == LODLevel::kContinuum);
  }
}
...
TEST(multiscale_drosophila_field_neuron_consistency) {
  ...
  // De-escalate: neuron rates should be written back to field
  engine.DeEscalate(0);
  assert(engine.chunks.empty());
  ...
  for (size_t i = 0; i < engine.grid.NumVoxels(); ++i) {
    ...
    assert(std::isfinite(E) && E >= 0.0f && E <= 1.0f);
    assert(std::isfinite(I) && I >= 0.0f && I <= 1.0f);
  }
}
...
TEST(population_model_steady_state) {
  ...
  assert(std::isfinite(s.r_e) && s.r_e > 0.0f);
  assert(std::isfinite(s.r_i) && s.r_i > 0.0f);
  assert(std::isfinite(s.v_e));
  assert(std::isfinite(s.v_i));
  ...
  assert(std::abs(s.r_e - r_e_ss) < 0.5f);
  assert(std::abs(s.r_i - r_i_ss) < 0.5f);
}
...
TEST(population_model_adaptation_reduces_rate) {
  ...
  PopulationParams p = PopulationParams::Drosophila();
  p.alpha_e = 10.0f;  // significant adaptation

  PopulationState s;

  // Step 20ms: capture early (onset) rate
  for (int i = 0; i < 20; ++i) {
    s.I_ext_e = 8.0f;
    PopulationStep(s, 1.0f, p);
  }
  ...
  // Step 2000ms more: adaptation should reduce rate
  for (int i = 0; i < 2000; ++i) {
    s.I_ext_e = 8.0f;
    PopulationStep(s, 1.0f, p);
  }
  ...
  // Adapted rate should be lower than onset
  assert(r_adapted < r_onset);
  assert(r_adapted > 0.0f);  // but not silenced
  assert(s.a_e > 0.0f);  // adaptation variable should be active
}
...
TEST(population_model_field_roundtrip) {
  ...
  assert(std::abs(fv.E - E_orig) < 0.05f);
  assert(std::abs(fv.I - I_orig) < 0.05f);
}
...
TEST(population_model_human_cortical_params) {
  ...
  assert(human.tau_e > fly.tau_e);      // slower pyramidal cells
  assert(human.alpha_e > fly.alpha_e);  // stronger adaptation
  assert(human.tau_a > fly.tau_a);      // slower adaptation
...
TEST(lod_transition_escalate_to_population) {
  ...
  assert(ok);
  assert(engine.pop_chunks.size() == 1);
  assert(engine.pop_chunks[0].name == "region_A");
  assert(engine.pop_chunks[0].nominal_neurons > 0);
  assert(!engine.pop_chunks[0].covered_voxels.empty());
  assert(engine.lod.region_lods[0].current_lod == LODLevel::kRegion);

  // LOD 2 and LOD 3 chunks should be empty
  assert(engine.chunks.empty());
  assert(engine.comp_chunks.empty());
}
...
TEST(lod_transition_deescalate_from_population) {
  ...
  // De-escalate back to field
  bool ok = engine.DeEscalateFromPopulation(0);
  assert(ok);
  assert(engine.pop_chunks.empty());
  assert(engine.lod.region_lods[0].current_lod == LODLevel::kContinuum);
...
TEST(lod_transition_spiking_to_population) {
  ...
  // Run some steps to accumulate rate data
  for (int i = 0; i < 100; ++i) engine.Step(0.1f, i * 0.1f);

  // De-escalate to LOD 1 (population)
  ...
  // Population state should be initialized from neuron ensemble
  auto& pop = engine.pop_chunks[0];
  assert(std::isfinite(pop.state.r_e));
  assert(std::isfinite(pop.state.v_e));
  assert(pop.state.r_e >= 0.0f);
}
...
TEST(lod_transition_population_step_runs) {
  ...
  for (const auto& pop : engine.pop_chunks) {
    assert(std::isfinite(pop.state.r_e));
    assert(std::isfinite(pop.state.v_e));
    assert(std::isfinite(pop.state.a_e));
    assert(std::isfinite(pop.state.r_i));
    assert(std::isfinite(pop.state.v_i));
    assert(std::isfinite(pop.state.a_i));
    assert(pop.state.r_e >= 0.0f);
    assert(pop.state.r_i >= 0.0f);
  }
}
...
TEST(lod_transition_population_field_coupling) {
  ...
  assert(engine.pop_chunks[0].state.I_ext_e > 0.0f);
...
TEST(lod_transition_population_read_activity) {
  ...
  float activity = engine.ReadActivity(25, 25, 25);
  assert(std::isfinite(activity));
  assert(activity >= 0.0f && activity <= 1.0f);

  // Should be approximately r_e / max_rate = 0.5
  assert(std::abs(activity - 0.5f) < 0.15f);
}

TEST(lod_transition_active_populations_count) {
  auto engine = MakeTestEngine();

  assert(engine.ActivePopulations() == 0);
  engine.EscalateToPopulation(0);
  assert(engine.ActivePopulations() == 1);
  engine.EscalateToPopulation(1);
  assert(engine.ActivePopulations() == 2);

  // TotalPopulationNeurons should be positive
  assert(engine.TotalPopulationNeurons() > 0);
}

TEST(lod_transition_full_4level_cycle) {
  auto engine = MakeTestEngine();

  // Full escalation: LOD 0 -> 1 -> 2 -> 3
  engine.EscalateToPopulation(0);
  assert(engine.lod.region_lods[0].current_lod == LODLevel::kRegion);
  for (int i = 0; i < 20; ++i) engine.Step(0.1f, i * 0.1f);
  ...
  assert(engine.lod.region_lods[0].current_lod == LODLevel::kNeuron);
  for (int i = 20; i < 40; ++i) engine.Step(0.1f, i * 0.1f);

  engine.EscalateToCompartmental(0);
  assert(engine.lod.region_lods[0].current_lod == LODLevel::kCompartmental);
  for (int i = 40; i < 50; ++i) engine.Step(0.025f, i * 0.025f);
...
TEST(lod_transition_automatic_population_escalation) {
  ...
  assert(engine.lod.region_lods[1].current_lod != LODLevel::kNeuron);
}
...
TEST(lod_transition_escalate_to_compartmental) {
  ...
  assert(engine.chunks.empty());
  assert(engine.comp_chunks.size() == 1);
  assert(engine.comp_chunks[0].name == "region_A");
  assert(engine.comp_chunks[0].neurons.n == n_neurons);
  assert(engine.lod.region_lods[0].current_lod == LODLevel::kCompartmental);

  // Positions should have been transferred
  assert(engine.comp_chunks[0].x.size() == n_neurons);
  assert(engine.comp_chunks[0].y.size() == n_neurons);
  assert(engine.comp_chunks[0].z.size() == n_neurons);

  // Synapse table should have been transferred
  assert(engine.comp_chunks[0].synapses.Size() > 0);
}
...
TEST(lod_transition_compartmental_soma_initialized_from_spiking) {
  ...
  for (size_t i = 0; i < comp.neurons.n; ++i) {
    assert(std::abs(comp.neurons.v_soma[i] - (-55.0f)) < 0.01f);
    // Dendrites should be at rest
    assert(std::abs(comp.neurons.v_apical[i] - comp.params.apical.E_leak) < 0.01f);
    assert(std::abs(comp.neurons.v_basal[i] - comp.params.basal.E_leak) < 0.01f);
  }
}
...
TEST(lod_transition_deescalate_from_compartmental) {
  ...
  assert(engine.comp_chunks.empty());
  assert(engine.chunks.size() == 1);
  assert(engine.chunks[0].neurons.n == n_neurons);
  assert(engine.lod.region_lods[0].current_lod == LODLevel::kNeuron);

  // Positions should be back
  assert(engine.chunks[0].neurons.x.size() == n_neurons);
...
TEST(lod_transition_compartmental_voltage_roundtrip) {
  ...
  for (size_t i = 0; i < engine.chunks[0].neurons.n; ++i) {
    ...
    assert(std::abs(engine.chunks[0].neurons.v[i] - expected) < 0.01f);
  }
}
...
TEST(lod_transition_compartmental_step_runs) {
  ...
  auto& comp = engine.comp_chunks[0];
  for (size_t i = 0; i < comp.neurons.n; ++i) {
    assert(std::isfinite(comp.neurons.v_soma[i]));
    assert(std::isfinite(comp.neurons.v_apical[i]));
    assert(std::isfinite(comp.neurons.v_basal[i]));
    assert(std::isfinite(comp.neurons.Ca_i[i]));
  }
  assert(comp.step_count > 0);
}
...
TEST(lod_transition_compartmental_deescalate_to_field) {
  ...
  // De-escalate all the way to field
...
TEST(lod_transition_automatic_compartmental_escalation) {
  ...
  int transitions = engine.Step(0.1f, 0.0f);
  assert(transitions >= 2);  // LOD 0 -> LOD 2 -> LOD 3
...
TEST(lod_transition_compartmental_total_neurons_and_spikes) {
  ...
  size_t total = engine.TotalActiveNeurons();
  assert(total == engine.chunks[0].neurons.n + engine.comp_chunks[0].neurons.n);
  assert(total > engine.comp_chunks[0].neurons.n);
}
...
TEST(lod_transition_compartmental_escalate_cycle) {
  ...
  // Full cycle: field -> spiking -> compartmental -> spiking -> field
  engine.Escalate(0);
...
TEST(cross_chunk_projection_instantiated) {
  ...
  engine.Escalate(0);
  assert(engine.ActiveCrossLinks() == 0);

  // Escalate region_B: cross link A->B should now exist
  engine.Escalate(1);
  assert(engine.ActiveCrossLinks() == 1);
  assert(engine.TotalCrossChunkSynapses() > 0);

  // Verify link direction
  assert(engine.cross_links[0].src_region == 0);
  assert(engine.cross_links[0].dst_region == 1);
}
...
TEST(cross_chunk_projection_no_duplicate) {
  ...
  assert(engine.ActiveCrossLinks() == links_before);
}
...
TEST(cross_chunk_bidirectional_projections) {
  ...
  assert(engine.ActiveCrossLinks() == 2);
...
TEST(coupling_population_to_population) {
  ...
  assert(pop_b.state.I_ext_e > 0.0f);
}
...
TEST(mixed_lod_step_all_levels) {
  ...
  assert(engine.lod.region_lods[0].current_lod == LODLevel::kContinuum);
  assert(engine.lod.region_lods[1].current_lod == LODLevel::kRegion);
  assert(engine.lod.region_lods[2].current_lod == LODLevel::kNeuron);
  assert(engine.lod.region_lods[3].current_lod == LODLevel::kCompartmental);
...
TEST(mixed_lod_step_produces_activity) {
  ...
  // Population should have non-zero activity
  auto& pop = engine.pop_chunks[engine.region_pop_map[0]];
  assert(pop.state.r_e > 0.0f);

  // Spiking region should have generated some spikes
  auto& chunk_b = engine.chunks[engine.region_chunk_map[1]];
  // At least some voxels should have accumulated spikes
  int total_accum = 0;
  for (auto c : chunk_b.voxel_spike_counts) total_accum += c;
  // With 100 steps of driven activity, expect some spikes
  assert(total_accum >= 0);  // conservative: just finite, not necessarily spiking
}
...
TEST(cross_chunk_step_integration) {
  ...
  for (size_t i = 0; i < chunk_b.neurons.n; ++i) {
    assert(std::isfinite(chunk_b.neurons.v[i]));
  }
}
...
TEST(neuromod_field_init) {
  ...
  assert(field.grid.nx > 0);
  assert(field.grid.ny > 0);
  assert(field.grid.nz > 0);
  assert(field.grid.channels.size() == 3);  // DA, 5HT, OA

  // All neurons should be in grid
  for (uint32_t i = 0; i < 10; ++i) {
    assert(field.neuron_in_grid[i]);
  }
}
...
TEST(neuromod_field_inject_and_sample) {
  ...
  for (uint32_t i = 0; i < neurons.n; ++i) {
    assert(neurons.dopamine[i] == 0.0f);
  }
...
TEST(neuromod_field_diffusion_spreads) {
  ...
  assert(da0_before > 0.0f);
  assert(da1_before == 0.0f);
...
TEST(neuromod_field_decay) {
  ...
  assert(std::abs(actual_ratio - expected_ratio) < 0.02f);
}
...
TEST(neuromod_field_serotonin_and_octopamine) {
  ...
  assert(neurons.serotonin[0] > 0.0f);
  assert(neurons.octopamine[0] > 0.0f);

  // Ratios should reflect injection amounts
  assert(neurons.serotonin[0] > neurons.dopamine[0]);
  assert(neurons.octopamine[0] > neurons.serotonin[0]);
}
