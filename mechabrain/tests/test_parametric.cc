...
TEST(scan_overlay_neurons) {
  ...
  for (size_t i = 0; i < 10; ++i) {
    assert(neurons.root_id[i] == 9000 + i);
    assert(std::abs(neurons.x[i] - 999.0f) < 0.01f);
    assert(neurons.type[i] == 7);
  }

  // Remaining 10 neurons should keep parametric values
  assert(neurons.root_id[10] != 9010);
}
...
TEST(scan_overlay_unknown_region) {
  ...
  auto result = ScanOverlay::Apply(sources, gen.region_ranges, neurons, synapses);
  assert(result.has_value());
  assert(*result == 0);  // skipped, not an error
}
...
TEST(brain_spec_loader_scan_fields) {
  ...
  assert(result.has_value());
  assert(result->regions[0].scan_neurons_path == "data/scan_neurons.bin");
  assert(result->regions[0].scan_synapses_path == "data/scan_synapses.bin");

  auto sources = BrainSpecLoader::CollectScanSources(*result);
  assert(sources.size() == 1);
  assert(sources[0].region_name == "scanned");
}
...
TEST(multiscale_mean_field_step) {
  ...
  assert(e_after >= 0.0f && e_after <= 1.0f);
  assert(bridge.mf_state[0].i >= 0.0f && bridge.mf_state[0].i <= 1.0f);
}
...
TEST(multiscale_region_scale_queries) {
  ...
  assert(bridge.IsMeanField("brain_a"));
  assert(!bridge.IsMeanField("brain_b"));
  assert(bridge.IsSpiking("brain_b"));
  assert(!bridge.IsSpiking("brain_a"));
  assert(bridge.IsSpiking("unknown"));  // default
}
...
TEST(mammalian_cell_type_params) {
  ...
  assert(std::abs(l23.a - 0.02f) < 0.001f);
  assert(std::abs(l23.c - (-65.0f)) < 0.01f);
  assert(std::abs(l23.v_thresh - 30.0f) < 0.01f);  // mammalian spike peak

  // L5 pyramidal: intrinsic bursting, c=-55 (shallower reset)
  auto l5 = ParamsForCellType(CellType::kL5_Pyramidal);
  assert(std::abs(l5.c - (-55.0f)) < 0.01f);
  assert(std::abs(l5.d - 4.0f) < 0.01f);

  // PV basket: fast spiking, a=0.1
  auto pv = ParamsForCellType(CellType::kPV_Basket);
  assert(std::abs(pv.a - 0.10f) < 0.001f);
  assert(std::abs(pv.d - 2.0f) < 0.01f);

  // SST Martinotti: low-threshold spiking, b=0.25
  auto sst = ParamsForCellType(CellType::kSST_Martinotti);
  assert(std::abs(sst.b - 0.25f) < 0.001f);

  // Thalamocortical: tonic mode, d=0.05 (minimal after-spike reset)
  auto tc = ParamsForCellType(CellType::kThalamocortical);
  assert(std::abs(tc.d - 0.05f) < 0.001f);
}

TEST(mammalian_cell_type_parsing) {
  // Verify BrainSpecLoader recognizes all mammalian types
  assert(BrainSpecLoader::ParseCellTypeName("L23_Pyramidal") == CellType::kL23_Pyramidal);
  assert(BrainSpecLoader::ParseCellTypeName("L2/3_Pyramidal") == CellType::kL23_Pyramidal);
  assert(BrainSpecLoader::ParseCellTypeName("L4_Stellate") == CellType::kL4_Stellate);
  assert(BrainSpecLoader::ParseCellTypeName("L5_Pyramidal") == CellType::kL5_Pyramidal);
  assert(BrainSpecLoader::ParseCellTypeName("L6_Pyramidal") == CellType::kL6_Pyramidal);
  assert(BrainSpecLoader::ParseCellTypeName("PV_Basket") == CellType::kPV_Basket);
  assert(BrainSpecLoader::ParseCellTypeName("PV") == CellType::kPV_Basket);
  assert(BrainSpecLoader::ParseCellTypeName("SST_Martinotti") == CellType::kSST_Martinotti);
  assert(BrainSpecLoader::ParseCellTypeName("SST") == CellType::kSST_Martinotti);
  assert(BrainSpecLoader::ParseCellTypeName("VIP_Interneuron") == CellType::kVIP_Interneuron);
  assert(BrainSpecLoader::ParseCellTypeName("VIP") == CellType::kVIP_Interneuron);
  assert(BrainSpecLoader::ParseCellTypeName("TC") == CellType::kThalamocortical);
  assert(BrainSpecLoader::ParseCellTypeName("TRN") == CellType::kTRN);
  assert(BrainSpecLoader::ParseCellTypeName("NBM") == CellType::kCholinergic_NBM);
}

TEST(mammalian_cell_type_labels) {
  // Verify NWB export labels for mammalian types
  assert(std::string(NWBExporter::CellTypeLabel(15)) == "L23_Pyramidal");
  assert(std::string(NWBExporter::CellTypeLabel(16)) == "L4_Stellate");
  assert(std::string(NWBExporter::CellTypeLabel(17)) == "L5_Pyramidal");
  assert(std::string(NWBExporter::CellTypeLabel(18)) == "L6_Pyramidal");
  assert(std::string(NWBExporter::CellTypeLabel(19)) == "PV_Basket");
  assert(std::string(NWBExporter::CellTypeLabel(20)) == "SST_Martinotti");
  assert(std::string(NWBExporter::CellTypeLabel(21)) == "VIP_Interneuron");
  assert(std::string(NWBExporter::CellTypeLabel(22)) == "Thalamocortical");
  assert(std::string(NWBExporter::CellTypeLabel(23)) == "TRN");
  assert(std::string(NWBExporter::CellTypeLabel(24)) == "Cholinergic_NBM");
}
...
TEST(cortical_column_spec_load) {
  ...
  assert(result.has_value());
  auto& spec = *result;
  assert(spec.name == "cortical_column_test");
  assert(spec.regions.size() == 3);
  assert(spec.regions[0].name == "L23");
  assert(spec.regions[1].name == "L4");
  assert(spec.regions[2].name == "thalamus");

  // Verify cell type fractions parsed correctly
  assert(spec.regions[0].cell_types.size() == 4);
  assert(spec.regions[0].cell_types[0].type == CellType::kL23_Pyramidal);
  assert(std::abs(spec.regions[0].cell_types[0].fraction - 0.80f) < 0.01f);
  assert(spec.regions[0].cell_types[1].type == CellType::kPV_Basket);
  assert(spec.regions[2].cell_types[0].type == CellType::kThalamocortical);

  assert(spec.projections.size() == 2);
  assert(spec.projections[0].from_region == "thalamus");
  assert(spec.projections[0].to_region == "L4");
}

TEST(cortical_column_generate_and_run) {
  // Generate a small cortical column and run it briefly
  BrainSpec spec;
  spec.name = "tiny_column";
  spec.seed = 42;
  spec.global_weight_mean = 0.8f;
  spec.global_weight_std = 0.3f;
  spec.background_current_mean = 8.0f;
  spec.background_current_std = 3.0f;
  ...
  l23.cell_types = {{CellType::kL23_Pyramidal, 0.80f},
                    {CellType::kPV_Basket, 0.10f},
                    {CellType::kSST_Martinotti, 0.10f}};
  l23.nt_distribution = {{kGlut, 0.80f}, {kGABA, 0.20f}};
  ...
  l5.cell_types = {{CellType::kL5_Pyramidal, 0.80f},
                    {CellType::kPV_Basket, 0.10f},
                    {CellType::kSST_Martinotti, 0.10f}};
  l5.nt_distribution = {{kGlut, 0.80f}, {kGABA, 0.20f}};
  ...
  tc.name = "thalamus";
  tc.n_neurons = 20;
  tc.internal_density = 0.10f;
  tc.cell_types = {{CellType::kThalamocortical, 1.0f}};
  tc.nt_distribution = {{kGlut, 1.0f}};
  spec.regions.push_back(tc);

  // Thalamus -> L5 projection
  ProjectionSpec proj;
  proj.from_region = "thalamus";
  proj.to_region = "L23";
  proj.density = 0.08f;
  proj.nt_type = kGlut;
  proj.weight_mean = 2.0f;
  spec.projections.push_back(proj);

  // L2/3 -> L5 projection
  ProjectionSpec proj2;
  proj2.from_region = "L23";
  proj2.to_region = "L5";
  proj2.density = 0.03f;
  proj2.nt_type = kGlut;
  spec.projections.push_back(proj2);
  ...
  assert(neurons.n == 160);
  assert(synapses.nnz > 0);

  // Verify heterogeneous cell type assignment
  int l23_pyr_count = 0, pv_count = 0, l5_pyr_count = 0, tc_count = 0;
  for (size_t i = 0; i < neurons.n; ++i) {
    auto ct = static_cast<CellType>(neurons.type[i]);
    if (ct == CellType::kL23_Pyramidal) l23_pyr_count++;
    if (ct == CellType::kPV_Basket) pv_count++;
    if (ct == CellType::kL5_Pyramidal) l5_pyr_count++;
    if (ct == CellType::kThalamocortical) tc_count++;
  }
  assert(l23_pyr_count > 0);
  assert(pv_count > 0);
  assert(l5_pyr_count > 0);
  assert(tc_count > 0);

  // Verify per-type Izhikevich params differ
  auto p_l23 = types.Get(0);  // first neuron should be L23 region
  auto p_tc_actual = types.Get(neurons.n - 1);  // last should be thalamus
  // L2/3 pyramidal: v_thresh=30 (mammalian)
  assert(std::abs(p_l23.v_thresh - 30.0f) < 0.01f ||
         std::abs(p_l23.v_thresh - 30.0f) < 1.0f);  // any mammalian type
  ...
  for (int step = 0; step < 1000; ++step) {
    float t = step * dt;
    IzhikevichStepHeterogeneous(neurons, dt, t, types);
    for (size_t i = 0; i < neurons.n; ++i) {
      total_spikes += neurons.spiked[i];
      neurons.i_syn[i] = 0.0f;
    }
    synapses.PropagateSpikes(neurons.spiked.data(), neurons.i_syn.data(), 1.0f);
  }

  // Mammalian neurons with background drive should fire
...
TEST(mammalian_homeostasis_targets) {
  ...
  assert(std::abs(TargetRateForCellType(15) - 5.0f) < 0.01f);   // L23_Pyramidal
  assert(std::abs(TargetRateForCellType(19) - 40.0f) < 0.01f);  // PV_Basket
  assert(std::abs(TargetRateForCellType(20) - 10.0f) < 0.01f);  // SST_Martinotti
  assert(std::abs(TargetRateForCellType(22) - 15.0f) < 0.01f);  // Thalamocortical
  assert(std::abs(TargetRateForCellType(24) - 3.0f) < 0.01f);   // Cholinergic_NBM
}
...
TEST(mammalian_rate_monitor_reference_lookup) {
  ...
  assert(mon.regions.size() == 1);
  assert(mon.regions[0].name == "L23");
  // Should find mammalian L23 reference: [1, 15] Hz
  assert(mon.regions[0].ref_min > 0.5f);
  assert(mon.regions[0].ref_max <= 20.0f);
}
...
TEST(species_defaults_brain_spec_integration) {
  ...
  assert(m.species == Species::kMouse);
  assert(std::abs(m.stp_tau_d - 200.0f) < 1.0f);
  // Mouse and Drosophila should differ in ref temperature
  assert(std::abs(m.ref_temperature_c - d.ref_temperature_c) > 10.0f);
}

TEST(species_parse_round_trip) {
  // ParseSpecies should handle all supported species strings
  assert(ParseSpecies("drosophila") == Species::kDrosophila);
  assert(ParseSpecies("fly") == Species::kDrosophila);
  assert(ParseSpecies("mouse") == Species::kMouse);
  assert(ParseSpecies("Mus musculus") == Species::kMouse);
  assert(ParseSpecies("rat") == Species::kRat);
  assert(ParseSpecies("human") == Species::kHuman);
  assert(ParseSpecies("Homo sapiens") == Species::kHuman);
  assert(ParseSpecies("zebrafish") == Species::kZebrafish);
  assert(ParseSpecies("Danio rerio") == Species::kZebrafish);
  assert(ParseSpecies("unknown") == Species::kGeneric);
}
