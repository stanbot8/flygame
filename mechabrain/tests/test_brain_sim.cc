...
TEST(brainsim_stdp) {
  ...
  CHECK(syn.weight[0] != old_w || true);  // just proves compilation
}
...
TEST(brainsim_homeostasis) {
  ...
  CHECK(h.per_neuron_target.size() == 10);
}
...
TEST(brainsim_cpg) {
  ...
  for (size_t i = 0; i < 20; ++i) {
    arr.region[i] = 5;
    arr.x[i] = (i < 10) ? 100.0f : 400.0f;  // split across midline (250)
  }
  CPGOscillator cpg;
  cpg.Init(arr, 5);
...
TEST(brainsim_motor_output) {
  ...
  CHECK(cmd.forward_velocity >= 0.0f || cmd.forward_velocity < 0.0f);  // just compiles
}
