...
struct AppConfig {
    ...
    // Use FK/IK stepper instead of CPG trajectory playback
    bool        use_stepper = false;

    // Use hybrid controller (CPG + cubic spline + corrections)
...
int main(int argc, char** argv) {
    ...
    NmfStepper nmf_stepper;
    nmf_stepper.Init(12.0);
    NmfHybrid nmf_hybrid;
    nmf_hybrid.Init(12.0);
    if (cfg.use_stepper)
        printf("[flygame] Using FK/IK stepper controller\n");
    if (cfg.use_hybrid)
        printf("[flygame] Using hybrid controller (cubic spline + corrections)\n");

    // Shoulder (coxa) joint gains: use MJCF defaults (45).
    // Lower values (e.g. 20) cause lateral drift from model asymmetries.

    // Get the standing neutral from CPG (tripod base + zero amplitude).
    double walk_ctrl[nmfly::kTotalActuators];
    memset(walk_ctrl, 0, sizeof(walk_ctrl));
    nmf_cpg.GetCtrl(walk_ctrl);  // fills 0..47 (42 joints + 6 adhesion)
    ...
    for (int i = 0; i < nmfly::kLegActuators; ++i)
        sim.data()->ctrl[i] = walk_ctrl[i];
    // Adhesion on
    for (int i = nmfly::kAdhesionStart; i < nmfly::kTotalActuators; ++i)
        sim.data()->ctrl[i] = 1.0;
    ...
    while (sim.IsRunning()) {
        ...
        if (cfg.use_brain && brain.IsInitialized()) {
            ...
            // Gate motor output on WASD input. Without WASD, zero all
            // motor channels so spontaneous brain activity doesn't
            // cause drift.
            if (!has_input && !brain_intent.flying) {
                cmd.forward_velocity = 0.0f;
                cmd.angular_velocity = 0.0f;
                cmd.approach_drive = 0.0f;
            } else {
                // Angular velocity: use direct WASD for responsive turning.
                // Brain's VNC L/R asymmetry is too noisy for clean control.
        ...
        // Map gait targets to MuJoCo actuators.
        {
            ...
            nmf_stepper.SetDrive(magnitude, turn);
            nmf_hybrid.SetDrive(magnitude, turn);

            // Wings disabled.
            nmf_wing.active = false;

            bool stable = true;
            double sub_dt = static_cast<double>(cfg.dt);
            for (int s = 0; s < cfg.substeps; ++s) {
                double nmf_ctrl[nmfly::kTotalActuators];
                memset(nmf_ctrl, 0, sizeof(nmf_ctrl));
                if (cfg.use_hybrid) {
                    nmf_hybrid.Step(sub_dt);
                    // Extract sensor data for hybrid corrections.
                    nmfly::HybridSensors hsens = {};
                    hsens.fly_z = sim.data()->qpos[2];
                    // End effector z: read tarsus5 body positions.
                    // For now, use contact-based corrections only.
                    for (int leg = 0; leg < 6; ++leg) {
                        hsens.leg_contact[leg] = false;
                        hsens.end_effector_z[leg] = hsens.fly_z;
                        hsens.contact_force_fwd[leg] = 0.0;
                    }
                    for (int c = 0; c < sim.data()->ncon; ++c) {
                        int g1 = sim.data()->contact[c].geom1;
                        int g2 = sim.data()->contact[c].geom2;
                        for (int leg = 0; leg < 6; ++leg) {
                            int gs = leg * 4, ge = gs + 4;
                            if ((gs <= g1 && g1 < ge) || (gs <= g2 && g2 < ge))
                                hsens.leg_contact[leg] = true;
                        }
                    }
                    nmf_hybrid.UpdateCorrections(sub_dt, hsens);
                    nmf_hybrid.GetCtrl(nmf_ctrl);
                } else if (cfg.use_stepper) {
                    nmf_stepper.Step(sub_dt);
                    nmf_stepper.GetCtrl(nmf_ctrl);
                } else {
                    nmf_cpg.Step(sub_dt);
                    nmf_cpg.GetCtrl(nmf_ctrl);
                }
                // Wing oscillation (overwrites ctrl[42..47]).
                nmf_wing.Update(static_cast<float>(sim.data()->time));
                nmf_wing.GetCtrl(nmf_ctrl);
                for (int i = 0; i < nmfly::kTotalActuators; ++i)
            ...
            if (!stable) {
                ...
                nmf_stepper.Init(12.0);
                nmf_hybrid.Init(12.0);
                double reset_ctrl[nmfly::kTotalActuators];
                ...
                if (cfg.standalone) {
                    nmf_cpg.SetDrive(1.0, 0.0);
                    nmf_stepper.SetDrive(1.0, 0.0);
                    nmf_hybrid.SetDrive(1.0, 0.0);
                }
            ...
            if (body_window && !cfg.headless) {
                ...
                if (show_panel) {
                    ...
                    if (ImGui::CollapsingHeader("Simulation",
                        ...
                        ImGui::Checkbox("IK Stepper", &cfg.use_stepper);
                        ImGui::Checkbox("Hybrid Controller", &cfg.use_hybrid);
